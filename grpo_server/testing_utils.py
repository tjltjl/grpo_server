...
class SimpleProblem:
    def __init__(self):
        ...
        self.dataset = datasets.Dataset.from_list(
            [dict(prompt=self.generate_prompt(rng)) for i in range(100)]
        )

    def generate_prompt(self, rng):
        while True:
            ...
            if np.all(np.array(self.tokenizer(prompt).input_ids) < self.n_vocab):
                return prompt
    ...
    def calculate_rewards(self, prompts, completions, **kwargs):
        """Reward: just keep repeating the last token."""
        # print(prompts, completions, kwargs)
        res = []
        for i, (p, c) in enumerate(zip(prompts, completions)):
            ids = self.tokenizer(p).input_ids + self.tokenizer(c).input_ids
            # print(ids)
            d = (np.diff(ids) == 0) + 0.0
            # print(d)
